...
7. `queryGraph.txt/matchGraph.txt`: Input format of the graph in RapidMatch. It starts with `t |V| |E|`, where V is the vertex set and E is the edge set. The vertices and edges are formatted as `v {vertex id} {vertex type} {vertex degree}` and `e {vertex id} {vertex id}`, respectively. The vertex id should start from 0 and the range should be limited within `[0,|V|-1]`.
