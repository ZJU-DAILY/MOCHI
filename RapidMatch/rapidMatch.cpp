...
//    uint64_t embedding_size = embedding_count * order_length * sizeof(uint32_t);
    std::ofstream embedding_file(path);
    embedding_file<<"matching order:"<<endl;
    for(int i=0;i<order_length-1;i++)
        embedding_file<<to_string(matching_order[i])+"-";
    embedding_file<<to_string(matching_order[order_length-1])<<endl;
    embedding_file<<"embeddings:"<<endl;
    string s="";
    for(int i=0;i<embedding_count* order_length;i++)
    {
        if((i+1)%order_length==0)
        {
            s=s+ to_string(embeddings[i]);
            embedding_file<<s<<endl;
            s="";
        }
        else
            s=s+ to_string(embeddings[i])+"-";
...
void rapidMatch::transformEmbedding(uint32_t *matching_order, uint32_t order_length,
                        uint32_t *embeddings, uint64_t embedding_count,result *matchRsl)
{
    ...
    map<int,int> maxID;
    long long count = 0;
    while(i<embedding_count*order_length)
    {
       if(i%order_length==0)
       {
           for(int j=0;j<order_length;j++)
               maxID[matching_order[j]]=-1;
           M1.clear();
           count++;
       }
        int vID=embeddings[i];
        int u=matching_order[i%order_length];
        if(vID>maxID[u])
        {
            M1[vID]=u;
            if(symmetricNodes->count(u))
            {
                for(int node:(*symmetricNodes)[u])
                    maxID[node]=vID;
                maxID[u]=vID;
            }
            if((i+1)%order_length==0)
            {
                matchRsl->vertexToMotif.push_back(M1);
                M1.clear();
            }
            i++;
        }
        ...
        {
            i=(i/order_length+1)*order_length;
        }
    }
    for(i=0;i<matchRsl->vertexToMotif.size();i++)
    {
        for(const auto&entry:matchRsl->vertexToMotif[i])
        {
            int vertex=entry.first;
//            cout<<to_string(vertex) + " ";
            if(matchRsl->vertexToMotifID.count(vertex) == 0)
            {
                set<int>newset;
                matchRsl->vertexToMotifID[vertex] = newset;
            }
            matchRsl->vertexToMotifID[vertex].insert(i);
            matchRsl->nodeGraph.insert(vertex);
        }
//        cout<<endl;
    }
    cout<<"count: " + to_string(count)<<endl;
    cout<<"instance num: " + to_string(matchRsl->vertexToMotif.size())<<endl;
}
void rapidMatch::transformEmbeddingForMMC(uint32_t *matching_order, uint32_t order_length,
                                    uint32_t *embeddings, uint64_t embedding_count,result *matchRsl)
{
    map<int,int>M1;
    vector<int>M;
    ...
    while(i<embedding_count*order_length)
    {
        ...
        if(vID>maxID[u])
        {
            ...
            {
//                matchRsl->vertexToMotif.push_back(M1);
                matchRsl->matched_subgraph.push_back(M);
                M1.clear();
                M.clear();
            }
            i++;
...
void rapidMatch::loadGraph(string data_graph_file)
{
    std::string input_data_graph_file = data_graph_file;
    auto data_graph = new Graph(true);
    data_graph->loadGraphFromFile(input_data_graph_file);
    this->data_graph=*data_graph;
}
bool rapidMatch::globalMatchForMMC(string query_graph_file,string data_graph_file,result *matchRsl,map<int, set<int>> *inputSymmetricNodes)
{
    symmetricNodes=inputSymmetricNodes;
    std::string input_query_graph_file = query_graph_file;
    std::string input_data_graph_file = data_graph_file;
    std::string input_order_type = "nd";
    std::string input_export_plan_path = "";
    config config;
    std::string input_max_embedding_num = to_string(config.motifNumLimit);
    std::string input_time_limit = to_string(config.matchTimeLimit);
    std::string input_order_num = "";
    std::string input_csr_file_path = "";
    std::string input_import_plan_path = "";
    std::string input_order = "";
    std::string input_enable_preprocessor = "true";
...
//    std::cout << "Enumerate..." << std::endl;
    ...
    if (input_max_embedding_num == "MAX") {
        ...
        output_limit=stoull(input_max_embedding_num);
//        sscanf(input_max_embedding_num.c_str(), "%zu", &output_limit);
    ...
    if (input_time_limit == "MAX") {
        ...
        time_limit=stoll(input_time_limit);
...
//    std::cout << "Release memories..." << std::endl;
    ...
    loadGraphTime=NANOSECTOSEC(load_graphs_time_in_ns);
    ...
    if(embedding_count>=config.motifNumLimit||NANOSECTOSEC(query_time)>=config.matchTimeLimit||embedding_count == 0)
        return false;
    auto start_time=std::chrono::high_resolution_clock::now();

    transformEmbeddingForMMC(spectrum[0].data(), spectrum[0].size(),
                       output_buffer, embedding_count,matchRsl);
    auto end_time=std::chrono::high_resolution_clock::now();
    chrono::duration<double> elapsed_time= end_time-start_time;
    cout<<"delete redundant embedding time: "+ to_string(elapsed_time.count())+"s"<<endl;
    delete[] output_buffer;
    return true;
}
bool rapidMatch::globalMatch(string query_graph_file,string data_graph_file,result *matchRsl,map<int, set<int>> *inputSymmetricNodes)
{
    symmetricNodes=inputSymmetricNodes;
    std::string input_query_graph_file = query_graph_file;
    std::string input_data_graph_file = data_graph_file;
...
//    std::cout << "Release memories..." << std::endl;
    ...
    transformEmbedding(spectrum[0].data(), spectrum[0].size(),
                       output_buffer, embedding_count,matchRsl);
    auto end_time=std::chrono::high_resolution_clock::now();
    chrono::duration<double> elapsed_time= end_time-start_time;
    cout<<"delete redundant embedding time: "+ to_string(elapsed_time.count())+"s"<<endl;
...
bool rapidMatch::globalMatchForGenerateQuery(string query_graph_file,result *matchRsl,map<int, set<int>> *inputSymmetricNodes)
{
    symmetricNodes=inputSymmetricNodes;
    std::string input_query_graph_file = query_graph_file;
//    std::string input_data_graph_file = data_graph_file;
...
//    std::cout << "Load graphs..." << std::endl;
    ...
    query_graph->loadGraphFromFile(input_query_graph_file);
    query_graph->buildCoreTable();
    Graph * data_graph=&this->data_graph;
//    auto data_graph = new Graph(true);
//    if (input_csr_file_path.empty()) {
//        data_graph->loadGraphFromFile(input_data_graph_file);
//    }
...
//    delete data_graph;
    ...
    std::cout << "End." << std::endl;
    if(embedding_count>=config.motifNumLimit||NANOSECTOSEC(query_time)>=config.matchTimeLimit)
        return false;
    auto start_time=std::chrono::high_resolution_clock::now();
    transformEmbedding(spectrum[0].data(), spectrum[0].size(),
                       output_buffer, embedding_count,matchRsl);
...
bool rapidMatch::globalMatchByMod(string query_graph_file,string data_graph_file,result *matchRsl,map<int, set<int>> *inputSymmetricNodes,int mod)
{
    symmetricNodes=inputSymmetricNodes;
    std::string input_query_graph_file = query_graph_file;
    std::string input_data_graph_file = data_graph_file;
